...
export const SettingsView: React.FC = () => {
  const { state, role, setRole, resetAuction, getViewerShareUrl, showNotification } = useAuction();
    ...
    <div className="space-y-6 pb-12 max-w-5xl mx-auto">
      ...
      <div className="p-5 md:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100">
            <span className="text-[10px] text-slate-500 block font-semibold uppercase">Zone / Village Quota</span>
            <span className="font-mono font-bold text-base text-slate-900 mt-1 block">
              {settings.maxVillageLimit >= 90 ? 'Unrestricted' : `Max ${settings.maxVillageLimit} / Zone`}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 md:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 text-xs font-semibold transition-colors"
          >
            Switch to {role === 'admin' ? 'Viewer Mode' : 'Admin Mode'}
          </button>
        </div>
      </div>
